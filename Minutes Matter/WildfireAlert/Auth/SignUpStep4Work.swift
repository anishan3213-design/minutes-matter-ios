...
struct SignUpStep4Work: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 24) {
            signUpScreenChrome(
                title: "Do you have a work location?",
                subtitle: "We'll alert you based on where you're most likely to be"
            )

            fieldLabel("Work address (optional)")
            TextField("", text: $workAddress, prompt: Text("Work address").foregroundColor(AppColors.textMuted))
            ...
            Button {
                ...
                Text("Skip — I work from home or skip")
                    .font(.system(size: 15))
        ...
        .onAppear {
            if let p = authState.profile {
                if workAddress.isEmpty { workAddress = p.workAddress ?? "" }
